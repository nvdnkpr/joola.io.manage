...
var nextCacheTime = new JustGage({
  id: "nextCacheTime",
  value: getRandomInt(1, 30),
  min: 1,
  max: 30,
  title: "Next eager cache run",
  label: "Minutes"
});

var cacheHitPct = new JustGage({
  id: "cacheHitPct",
  value: getRandomInt(0, 100),
  min: 1,
  max: 100,
  title: "Cache Hits",
  symbol: "%",
  levelColors: [ "#F27C07", "#ff0000", "#a9d70b" ]
});

var actionData = [
  { label: "Cache Query", data: 12},
  { label: "Page load", data: 27},
  { label: "Source Query", data: 85},
  { label: "Auth", data: 64},
  { label: "Log", data: 90},
  { label: "Config", data: 112}
];
$.plot($("#actionDonut"), actionData,
  {
    series: {
      pie: {
        radius:300,
...
setInterval(function () {
  cacheHitPct.refresh(getRandomInt(1, 100));
}, 3000);
setInterval(function () {
  nextCacheTime.refresh(getRandomInt(1, 30));
}, 3000);