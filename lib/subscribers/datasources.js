...
joola.dispatch.once('datasources', 'update-request', function (err, ds) {
  joola.logger.debug('Update datasource request ' + ds.name);
  return datasources.update(ds);
});

joola.dispatch.once('datasources', 'update-done', function (err, ds) {
  if (err)
    return joola.logger.error('Failed to update datasource: ' + err);

  joola.logger.debug('Updated datasource [' + ds.name + ']');
});


joola.dispatch.once('datasources', 'delete-request', function (err, ds) {
  joola.logger.debug('Delete datasource request ' + ds.name);
